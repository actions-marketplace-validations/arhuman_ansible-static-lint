...
var equivalence = []idPair{
	...
	{"latest", "task.unpinned-checkout", "every version control checkout pinning check"},
	{"latest[git]", "task.unpinned-checkout[git]", "a `git` checkout tracks `HEAD` instead of a commit or tag"},
	{"latest[hg]", "task.unpinned-checkout[hg]", "an `hg` checkout tracks the default branch instead of a revision"},
	...
	{"no-free-form", "task.free-form", "every free-form action check"},
	{"no-free-form[raw]", "task.free-form[raw]", "a `raw` call embeds `executable=` instead of passing explicit args"},
	{"no-free-form[raw-non-string]", "task.free-form[raw-non-string]", "a `raw` call is given a value that is not a string"},
